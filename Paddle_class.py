...
class Player_paddle(Paddle):
    def update(self):
        ...
        if key_mods[pygame.K_w] and (not key_mods[pygame.K_s]) and (not self.pos[1] < globals.SCREEN_PADDING):
            self.move(-self.speed)
        if key_mods[pygame.K_s] and (not key_mods[pygame.K_w]) and (not (self.pos[1] + self.size[1]) > (globals.SCREEN_HEIGHT - globals.SCREEN_PADDING)):
            self.move(self.speed)
